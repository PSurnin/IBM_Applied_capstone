...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                # The default select value is for ALL sites
                                   html.Div(["Input Year: ", dcc.Dropdown(id='site-dropdown',
                                    options=[
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df['name'] = ['Failed', 'Success']
...
def get_scatter(entered_site, payload):
    payload_df = spacex_df[(payload[0] < spacex_df['Payload Mass (kg)'])  & (spacex_df['Payload Mass (kg)'] < payload[1])]
    filtered_df = payload_df[payload_df['Launch Site']== entered_site]
    if entered_site == 'ALL':
        fig = px.scatter(payload_df, x='Payload Mass (kg)',
              y='class', title='Total Success Launches by site',
